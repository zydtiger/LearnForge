 ...
 */
function convertMenuToDisplayMode(
  srcItems: MenuProps["items"],
): MenuProps["items"] {
  const items = [...srcItems!];
  const levelContainsChildren = items
    .map((item) => "children" in item!)
    .reduce((acc, cur) => (acc ||= cur));
  for (const item of items) {
    if (levelContainsChildren) {
      if ("children" in item!)
        item.children = convertMenuToDisplayMode(item.children);
...
export {
  actions,
  convertMenuToDisplayMode,
  convertToPlatformShortcuts,
  bindShortcuts,
  invokeAction,
};
