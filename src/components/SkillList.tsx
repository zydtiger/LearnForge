...
function SkillList() {
  ...
  const handleOnChange = (event: SyntheticEvent) => {
    ...
    if (NodeEventTypes.includes(eventType)) {
      // @ts-ignore
      handleNodeChange(id, eventType, event.value);
    } else {
  ...
  const handleOnDrop: TreeProps["onDrop"] = (info) => {
    ...
    const [dragSiblings, dragIndex, dragNode] = findListNode(
      listDataClone,
      dragKey,
    )!;
